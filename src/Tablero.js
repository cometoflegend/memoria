...
function Tablero() {
    ...
    const [cartasFlippeadas, setCartasFlippeadas] = useState([]);
    const [cartasMatcheadas, setCartasMatcheadas] = useState([]);

    const handleCartaClick = (id) => {
        if (cartasFlippeadas.length === 2) return; // No se hace nada si las cartas ya están flippeadas
    
        const carta = cartas.find(carta => carta.id === id);
    
        // Si la carta ya está emparejada, no se hace nada
        if (cartasMatcheadas.includes(id) || cartasFlippeadas.includes(id)) return;
    
        // Voltear cartas
        setCartasFlippeadas(prev => [...prev, id]);
    
        // Si ya hay dos cartas volteadas, se verifica si coinciden
        if (cartasFlippeadas.length === 1) {
          const firstCarta = cartas.find(carta => carta.id === cartasFlippeadas[0]);
        ...
        <div className="grid">
          {cartas.map((carta) => (
            <Carta
              ...
              id={carta.id}
              value={carta.value}
              isFlipped={cartasFlippeadas.includes(carta.id) || cartasMatcheadas.includes(carta.id)}
